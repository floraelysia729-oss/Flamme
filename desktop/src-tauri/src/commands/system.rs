...
// ── MCP commands (desktop, tolaria-ai only) ─────────────────────────────────

#[cfg(all(desktop, feature = "tolaria-ai"))]
#[tauri::command]
...
// ── Flamme sidecar (desktop, default flamme build) ───────────────────────────

#[derive(serde::Serialize)]
#[serde(rename_all = "camelCase")]
pub struct FlammeSidecarConfig {
    pub dev_skip_spawn: bool,
    pub port: u16,
}

#[cfg(all(desktop, not(feature = "tolaria-ai")))]
#[tauri::command]
pub async fn get_flamme_sidecar_config() -> Result<FlammeSidecarConfig, String> {
    Ok(FlammeSidecarConfig {
        dev_skip_spawn: crate::flamme_sidecar::flamme_dev_skip_spawn(),
        port: 8765,
    })
}
...
#[cfg(all(desktop, not(feature = "tolaria-ai")))]
#[tauri::command]
pub async fn spawn_flamme_sidecar_command(
    app: tauri::AppHandle,
    vault_path: String,
) -> Result<String, String> {
    let expanded = super::expand_tilde(vault_path.trim());
    if expanded.trim().is_empty() {
        return tokio::task::spawn_blocking(move || crate::sync_flamme_sidecar_for_vault(&app, None))
            .await
            .map_err(|e| format!("spawn_flamme_sidecar task failed: {e}"))?;
    }
    let path_buf = std::path::PathBuf::from(expanded.into_owned());
    tokio::task::spawn_blocking(move || {
        crate::sync_flamme_sidecar_for_vault(&app, Some(path_buf.as_path()))
    })
    .await
    .map_err(|e| format!("spawn_flamme_sidecar task failed: {e}"))?
}
...
#[cfg(all(desktop, not(feature = "tolaria-ai")))]
#[tauri::command]
pub async fn stop_flamme_sidecar_command(app: tauri::AppHandle) -> Result<String, String> {
    tokio::task::spawn_blocking(move || crate::sync_flamme_sidecar_for_vault(&app, None))
        .await
        .map_err(|e| format!("stop_flamme_sidecar task failed: {e}"))?
}

#[cfg(any(not(desktop), feature = "tolaria-ai"))]
#[tauri::command]
pub async fn get_flamme_sidecar_config() -> Result<FlammeSidecarConfig, String> {
    Ok(FlammeSidecarConfig {
        dev_skip_spawn: true,
...
pub async fn spawn_flamme_sidecar_command(
    ...
    Err("Flamme sidecar is not available in this build".into())
}

#[cfg(any(not(desktop), feature = "tolaria-ai"))]
#[tauri::command]
pub async fn stop_flamme_sidecar_command(_app: tauri::AppHandle) -> Result<String, String> {
    Err("Flamme sidecar is not available in this build".into())
}
