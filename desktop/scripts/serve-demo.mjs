#!/usr/bin/env node
/**
 * 生产静态服务器 + Vault HTTP API（浏览器 Demo 用）
 *
 * 用途：在不启动 Tauri 的情况下，用浏览器访问打包后的 dist/ 并操作 vault
 *
 * 路由：
 *   GET  /api/vault/ping          — 健康检查（mock-tauri 用来检测 API 是否可用）
 *   GET  /api/vault/list          — 列出 vault 条目
 *   GET  /api/vault/content       — 读取单篇笔记内容
 *   POST /api/vault/save          — 保存笔记
 *   POST /api/vault/delete        — 删除笔记
 *   ... 等（见下方 handler）
 *
 * 与 Python 后端的关系：
 *   这是 Node 版 vault API 原型；mock-tauri/vault-api.ts 会代理 Tauri 命令到此。
 *   若要换 Python，实现相同 /api/vault/* 契约即可，前端无需改 Tauri 路径。
 *
 * 启动：node scripts/serve-demo.mjs  （默认端口 5173）
 */
