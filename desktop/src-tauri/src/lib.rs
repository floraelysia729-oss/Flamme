...
#[cfg(all(desktop, not(feature = "tolaria-ai")))]
pub(crate) fn sync_flamme_sidecar_for_vault(
    app_handle: &tauri::AppHandle,
    vault_path: Option<&Path>,
) -> Result<String, String> {
    use tauri::Manager;

    if flamme_sidecar::flamme_dev_skip_spawn() {
        return Ok("skipped_dev".into());
    }

    let state: tauri::State<'_, FlammeSidecarChild> = app_handle.state();
    let mut guard = state
        .0
        .lock()
        .map_err(|_| "Failed to lock Flamme sidecar state".to_string())?;

    let Some(vault_path) = vault_path else {
        flamme_sidecar::stop_flamme_sidecar(&mut guard);
        return Ok("stopped".into());
    };

    let expanded = commands::expand_tilde(
        vault_path
            .to_str()
            .ok_or_else(|| "Vault path is not valid UTF-8".to_string())?,
    );
    let vault_path = Path::new(expanded.as_ref());

    flamme_sidecar::stop_flamme_sidecar(&mut guard);

    let child = flamme_sidecar::spawn_flamme_sidecar(vault_path)?;
    *guard = Some(child);

    match flamme_sidecar::wait_healthy(flamme_sidecar::default_health_wait()) {
        Ok(()) => Ok("healthy".into()),
        Err(e) => {
            log::warn!("Flamme sidecar health check failed: {e}");
            Ok("started_unhealthy".into())
        }
    }
}
    ...
    F: FnOnce(PathBuf) + Send + 'static,
{
    ...
    spawn_background_task("flamme-startup-tasks", move || task(vault_path));
    true
}
...
fn spawn_initial_ws_bridge_sync(app: &tauri::App) {
    ...
    spawn_background_task("flamme-ws-bridge-startup", move || {
...
macro_rules! app_invoke_handler {
    () => {
        tauri::generate_handler![
            ...
            commands::create_note_from_agent,
            ...
            #[cfg(all(desktop, not(feature = "tolaria-ai")))]
            commands::spawn_flamme_sidecar_command,
            #[cfg(all(desktop, not(feature = "tolaria-ai")))]
            commands::stop_flamme_sidecar_command,
            #[cfg(all(desktop, not(feature = "tolaria-ai")))]
            commands::get_flamme_sidecar_config,
...
pub fn run() {
    ...
    let builder = {
        ...
        #[cfg(not(feature = "tolaria-ai"))]
        let builder = builder.manage(FlammeSidecarChild(Mutex::new(None)));
        builder
