...
import { useFlammeRuntime } from './hooks/useFlammeRuntime'
import { useFlammeHeaders } from './hooks/useFlammeHeaders'
...
function MainApp({ noteWindowParams }: { noteWindowParams: NoteWindowParams | null }) {
  ...
  const flammeHeaders = useFlammeHeaders(settings)
  const flammeRuntime = useFlammeRuntime(resolvedPath, flammeHeaders)
  const flammeEmbeddingProgress = flammeRuntime.status?.embeddings ?? null
  const isGraphSelection = effectiveSelection.kind === 'filter' && effectiveSelection.filter === 'graph'
  const flammeGraphEnabled = aiFeaturesEnabled && flammeRuntime.apiReachable
  const flammeAgentReadiness = flammeRuntime.state === 'sidecar_starting'
    ? 'checking' as const
    : flammeRuntime.isChatReady
      ? 'ready' as const
      : 'missing' as const
  const flammeChatReady = aiFeaturesEnabled && flammeRuntime.isChatReady

  useEffect(() => {
    if (!effectiveShowAIChat || !aiFeaturesEnabled) return
    void flammeRuntime.ensureChatEmbeddings(flammeRuntime.status)
  }, [effectiveShowAIChat, aiFeaturesEnabled, flammeRuntime])

  useEffect(() => {
    if (!isGraphSelection || !aiFeaturesEnabled) return
    void flammeRuntime.ensureGraphBuilt()
  }, [isGraphSelection, aiFeaturesEnabled, flammeRuntime])
          ...
          <div className={`app__editor${aiActivity.highlightElement === 'editor' || aiActivity.highlightElement === 'tab' ? ' ai-highlight' : ''}`}>
            {isGraphSelection ? (
              <GraphPanel
                vaultPath={resolvedPath}
                keys={flammeHeaders}
                onOpenNote={(path) => { void notes.handleNavigateWikilink(path) }}
                onEnsureGraphBuilt={flammeRuntime.ensureGraphBuilt}
              />
            ) : (
            <Editor
              tabs={notes.tabs}
              ...
              defaultAiAgentReadiness={aiFeaturesEnabled ? flammeAgentReadiness : aiAgentPreferences.defaultAiAgentReadiness}
              defaultAiAgentReady={aiFeaturesEnabled ? flammeChatReady : aiAgentPreferences.defaultAiAgentReady}
              ...
              locale={appLocale}
              flammeGraphEnabled={flammeGraphEnabled}
              flammeKeys={flammeHeaders}
              markInternalWrite={markRecentVaultWrite}
            />
            )}
          </div>
