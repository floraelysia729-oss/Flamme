...
pub enum ClaudeStreamEvent {
    /// Session initialised â€?carries the session ID for future `--resume`.
    ...
    F: FnMut(ClaudeStreamEvent),
{
    ...
    match msg_type {
        // --- System init â†?capture session_id ---
...
mod tests {
    ...
    fn dispatch_stream_event_input_json_delta_accumulates_silently() {
        // input_json_delta doesn't emit events directly â€?it accumulates in state
        let (_, events) = run_dispatch(serde_json::json!({
            "type": "stream_event",
