 ...
 * Tauri IPC 命令封装层
 *
 * 所有前端 → Rust 后端的调用都经此文件，统一处理：
 *   - 桌面环境：invoke(command, tauriArgs) 走 Tauri IPC
 *   - 浏览器环境：mockInvoke 走 mock-tauri（含 HTTP vault-api 代理）
 *
 * 对应 Rust 侧：src-tauri/src/commands/ 中 #[tauri::command] 函数
 */
...
/** 统一 IPC 入口：Tauri 桌面走 invoke，浏览器走 mock（含 HTTP 代理） */
