...
#[tauri::command]
pub fn create_note_from_agent(
    relative_path: String,
    content: String,
    mode: Option<String>,
    vault_path: Option<PathBuf>,
) -> Result<(), String> {
    let Some(vault_root) = vault_path else {
        return Err("vault_path is required for agent note writes".into());
    };

    let normalized_relative = relative_path.replace('\\', "/");
    let joined = vault_root.join(normalized_relative.replace('/', std::path::MAIN_SEPARATOR_STR));
    let write_mode = mode.as_deref().unwrap_or("create");

    with_writable_note_path(joined, Some(vault_root), |validated_path| {
