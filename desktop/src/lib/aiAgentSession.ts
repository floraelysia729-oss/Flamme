...
import {
  ...
  createFlammeUnavailableResponse,
  type AgentStatus,
...
import { USE_FLAMME_CHAT } from './flamme/constants'
import { streamFlammeChat } from './flamme/sse-adapter'
...
function blockUnavailableAgent(runtime: AiAgentSessionRuntime, context: AgentExecutionContext, prompt: PendingUserPrompt): void {
  ...
  appendLocalResponse(
    runtime.setMessages,
    prompt,
    USE_FLAMME_CHAT ? createFlammeUnavailableResponse() : createMissingAgentResponse(context.agent),
  )
}
...
async function streamWithSelectedTarget(
  ...
  if (USE_FLAMME_CHAT) {
    if (context.target?.kind === 'api_model') {
      callbacks.onError('Phase 1 仅支持 Flamme 引擎，请在设置中切换为 Agent 模式。')
      callbacks.onDone()
      return
    }

    await streamFlammeChat({
      message: formattedMessage,
      sessionId: context.sessionId ?? crypto.randomUUID(),
      vaultPath: context.vaultPath,
      selectedFiles: context.selectedFiles,
      keys: context.flammeKeys,
      callbacks,
      fileCallbacks,
      markInternalWrite: context.markInternalWrite,
    })
    return
  }
...
export async function sendAgentMessage({
  ...
  await streamWithSelectedTarget(
    ...
    systemPrompt,
    callbacks,
    runtime.fileCallbacksRef.current,
  )
}
