...
#[cfg(not(feature = "tolaria-ai"))]
use ai_models_compat::{normalize_ai_model_providers, AiModelProvider};
...
const SUPPORTED_AI_CHAT_ENGINES: &[&str] = &["flamme", "external"];
...
pub struct Settings {
    ...
    pub ai_chat_engine: Option<String>,
    ...
    pub flamme_llm_key: Option<String>,
    pub flamme_embed_key: Option<String>,
    pub flamme_brain_key: Option<String>,
    pub flamme_mineru_token: Option<String>,
}
...
pub fn normalize_ai_chat_engine(value: Option<&str>) -> Option<String> {
    match value.map(|candidate| candidate.trim().to_ascii_lowercase()) {
        Some(engine) if SUPPORTED_AI_CHAT_ENGINES.contains(&engine.as_str()) => Some(engine),
        _ => None,
    }
}
...
fn normalize_settings(settings: Settings) -> Settings {
    Settings {
        ...
        ai_chat_engine: normalize_ai_chat_engine(settings.ai_chat_engine.as_deref()),
        ...
        flamme_llm_key: normalize_optional_string(settings.flamme_llm_key),
        flamme_embed_key: normalize_optional_string(settings.flamme_embed_key),
        flamme_brain_key: normalize_optional_string(settings.flamme_brain_key),
        flamme_mineru_token: normalize_optional_string(settings.flamme_mineru_token),
    }
}
