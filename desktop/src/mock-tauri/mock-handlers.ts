...
export const mockHandlers: Record<string, (args: any) => any> = {
  ...
  create_note_from_agent: (args: {
    relativePath: string
    content: string
    mode?: string
    vaultPath?: string
  }) => {
    const vault = args.vaultPath ?? '/Users/luca/Laputa'
    const absolutePath = `${vault.replace(/\/+$/, '')}/${args.relativePath.replace(/^\/+/, '')}`
    if (args.mode === 'update') {
      MOCK_CONTENT[absolutePath] = args.content
    } else {
      MOCK_CONTENT[absolutePath] = args.content
    }
    mockSavedSinceCommit.add(absolutePath)
    syncWindowContent()
    return null
  },
