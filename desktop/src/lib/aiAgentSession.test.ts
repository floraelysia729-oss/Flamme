...
const {
  ...
  streamFlammeChatMock: vi.fn(async () => {}),
...
vi.mock('./flamme/sse-adapter', () => ({
  streamFlammeChat: streamFlammeChatMock,
}))
...
function expectStreamingRequest(runtime: RuntimeFixture['runtime']): void {
  ...
  expect(streamFlammeChatMock).toHaveBeenCalledWith({
  ...
  it('appends local fallback responses when the session cannot stream', async () => {
    const fallbackCases = [
      ...
      },
      {
        ...
        response:
          'Flamme AI 引擎未就绪。请等待启动完成，或在开发模式下运行 scripts/dev-flamme.ps1 启动 Python 后端。',
      },
    ] as const
